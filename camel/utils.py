...
import tiktoken
import time
from camel.model_config import MODELS
...
def get_model_info(platform, model_name, api_type):
    ...
    """
    # 检查 platform 是否在 MODELS 的键中
    if platform not in MODELS.keys():
        raise ValueError("平台错误，应从 {}".format(MODELS.keys()))
    
    # 检查 api_type 是否在指定平台的 api_types 列表中
    if api_type not in MODELS[platform]["api_types"]:
        raise ValueError("平台.api_type 错误，应从 {}.{}".format(platform, MODELS[platform]["api_types"]))
    
    # 获取指定平台的 base_url
    base_url = MODELS[platform]["base_url"]
    num_tokens = None
    
    # 遍历指定平台的 models 列表，查找与 model_name 匹配的模型
    for model_info in MODELS[platform]["models"]:
        if model_info["name"] == model_name:
            num_tokens = model_info["num_tokens"]
            break
    
    # 如果没有找到匹配的模型名称，抛出异常并提供可选的模型名称列表
    if not num_tokens:
        model_list = [model_info["name"] for model_info in MODELS[platform]["models"]]
        raise ValueError("模型名称错误，应从以下选项中选择 {}".format(model_list))
    
    # 返回 base_url, num_tokens 和 api_type
    return base_url, num_tokens, api_type
...
def get_model_token_limit(model: ModelType) -> int:
    ...
    if model == ModelType.GPT_3_5_TURBO:
        ...
        return 128000
    elif model == ModelType.GPT_3_5_TURBO_NEW_1:
        return 4196
    else:
