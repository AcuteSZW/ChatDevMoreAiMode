...
class CodeReviewHuman(Phase):
    ...
    def execute(self, chat_env, chat_turn_limit, need_reflect) -> ChatEnv:
        ...
        log_visualize(
            f"**[人机交互]**\n\n"
            f"现在您可以参与软件开发了！\n"
            f"任务是:  {chat_env.env_dict['task_prompt']}\n"
            f"请输入您的反馈（多行输入）。可以是错误报告或新功能需求。\n"
            f"您当前处于第 {self.phase_env['cycle_index']} 次人类反馈，总共 {self.phase_env['cycle_num']} 次反馈\n"
            f"在单独一行输入 'end' 提交。\n"
            f"您可以随时输入 \"Exit\" 退出此模式。\n"
        )
        provided_comments = []
        while True:
            user_input = input(">>>>>>")
